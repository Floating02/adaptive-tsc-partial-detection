...
本模块实现了两个观测类，用于交通信号控制系统：
- PartialObservationFunction: 部分检测车辆（可配置检测率）
- FullDetectionObservationFunction: 完全检测车辆（100%检测率）

状态表示包括：车辆数量、最近车辆距离、当前相位时间、黄灯指示器、当前时间等信息。
"""
...
class FullDetectionObservationFunction(ObservationFunction):
    """无部分检测的完全观测类（与PartialObservationFunction相同的特征结构，但检测率为100%）

    状态表示包括（所有特征均已归一化）：
    - 车辆数量（带符号）：每个车道上所有车辆的数量，除以车道最大容量；
      绿灯车道为正值，红灯车道为负值，范围 -1 到 1
    - 最近车辆距离（带符号）：每个车道上最近车辆的距离，除以车道长度；
      绿灯车道为正值，红灯车道为负值，范围 -1 到 1
    - 当前相位时间：从当前相位开始到现在的持续时间（秒），除以最大绿灯时间，范围 0-1
    - 黄灯指示器：黄灯相位指示器（0或1）
    - 当前时间：一天中的当前时间，使用正弦和余弦编码为2维向量，范围 -1 到 1

    与PartialObservationFunction的区别：
    - 始终检测所有车辆（无检测率过滤，无_randomness）
    - 不需要detected_vehicles字典跟踪
    - 不需要_update_detected_vehicles()方法
    ...
    def __init__(self, ts: TrafficSignal, max_car_capacity: int = 10, max_phase_duration: int = 120):
        """初始化完全检测的观察函数
    ...
    def __call__(self) -> np.ndarray:
        ...
        for lane in self.ts.lanes:
            ...
            normalized_count = min(vehicle_count / self.max_car_capacity, 1.0)

            lane_length = self.ts.lanes_length[lane]
            if veh_list:
                positions = [lane_length - self.ts.sumo.vehicle.getLanePosition(veh) for veh in veh_list]
                min_distance = min(positions)
            else:
                min_distance = lane_length
            normalized_distance = min_distance / lane_length
    ...
    def observation_space(self) -> spaces.Box:
        ...
        观察空间的维度由以下部分组成：
        - 各车道车辆数量（带符号，绿灯为正、红灯为负，-1到1）
        - 各车道最近车辆距离（带符号，绿灯为正、红灯为负，-1到1）
        - 当前相位时间 (0-1)
        - 黄灯相位指示器 (0或1)
        - 当前时间编码 (2维，-1到1)
